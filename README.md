...
| 字段名 | 英文名 | 数据类型 | 示例值 | 说明 |
|--------|----------|-----------|--------|------|
| 订单编号 | `order_id` | string | `ORD_20250101_001` | 每个订单的唯一标识符 |
| 区域 | `region` | string | `East China` | 订单所属的物流区域 |
| 成本 | `cost` | float | `128.5` | 订单总成本（元） |
| 配送时间 | `delivery_time` | float | `5.3` | 从发货到收货的时间（小时） |
| 车辆类型 | `vehicle_type` | string | `Truck` | 执行该订单的运输车辆类型 |
| 重量 | `weight` | float | `23.4` | 货物重量（kg） |
| 距离 | `distance` | float | `120.8` | 运输距离（公里） |
| 发货仓 | `warehouse` | string | `Shanghai_WH_01` | 发货仓库编号 |
| 目的地城市 | `destination_city` | string | `Hangzhou` | 订单目的地城市 |
| 状态 | `status` | string | `Delivered` | 当前订单状态（如：Delivered, Pending, Cancelled） |
